...
This is Ritika Sibal's R Markdown File for Modules 3,4 & 5. Work will be seperated by modules, and relevant notes have been added. 

## Module 4 Notes
The following work is for Module 4.
```{r Module 4}
m<-matrix(data=c(1,2,3,4), nrow=2, ncol=2)
```
Matrix is a collection of vectors stored together, can only have one data type
...
Data frames are good at showing tabular data. Use *cbind()* or *rbind()* to add more rows or cols. Note that the frame is more efficient for strings because internally R assigns int values to the unique string (like an enum)
